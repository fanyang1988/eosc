...
You can then use this vault for the different eosc operations.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if wrapType == "kms-gcp" && kmsGCPKeypath == "" {
			utils.ErrorCheck("missing parameter", fmt.Errorf("--kms-gcp-keypath is required with --vault-type=kms-gcp"))
		}
		...
		if doImport {
			...
			utils.ErrorCheck("entering private key", err)
			...
			for i := 0; i < numKeys; i++ {
				...
				utils.ErrorCheck("creating new keypair", err)
		...
		case "passphrase":
			...
			utils.ErrorCheck("password input", err)
		...
		utils.ErrorCheck("sealing vault", vault.Seal(boxer))
		utils.ErrorCheck("writing wallet file", vault.WriteToFile(walletFile))
